...
class ResponseStream {
  ...
  flush(messageId) {
    ...
    if (stream.firstDeltaAt == null) {
      ...
      performanceTracker.mark('response-first-delta', {
        messageId,
    ...
    performanceTracker.mark('response-stream-delta', {
      messageId,
      sequence: stream.sequence,
      deltaLength: delta.length
    });
    ...
    stream.closed = true;
    if (error) {
      stream.emit('error', this._envelope(stream, {
        error: String(error),
        content: '',
        sequence: stream.sequence
      }));
      ...
      stream.emit('end', this._envelope(stream, {
